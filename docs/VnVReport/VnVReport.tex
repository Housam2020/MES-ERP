...
\begin{longtable}{|p{0.45\textwidth}|p{0.45\textwidth}|}
  ...
  \hline
  Login with valid credentials & User is authenticated and redirected to dashboard \\
  \hline
  Login with invalid credentials & System displays error message and remains on login page \\
  \hline
  Register with new email & User account is created and user is redirected to dashboard \\
  \hline
  Register with existing email & System prevents duplicate registration with error message \\
  \hline
  Email confirmation flow & System prompts user to check email when confirmation is required \\
  \hline
\end{longtable}
...
\begin{itemize}
\item Fixed an issue where authentication redirect wasn't working correctly for users with expired sessions
\emph{(discovered during redirect testing; implemented proper token validation)}.
\item Addressed a bug where permission checks were not correctly filtering dashboard content
\emph{(observed during permissions testing; refined access control implementation)}.
\item Resolved an issue where loading states would persist indefinitely under certain conditions
\emph{(discovered during component mount testing; improved error boundary handling)}.
\item Fixed UI rendering inconsistencies in group management interfaces
\emph{(component tests revealed incorrect state updates; implemented proper React state management)}.
\item Corrected the behavior of user role assignment when users belong to multiple groups
\emph{(unit tests highlighted logic errors; updated permission assignment logic)}.
\end{itemize}
